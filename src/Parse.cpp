...
eInstructions	Parse::get_instruction(std::string const &line, std::string::size_type &pos) {
	...
	else if (line.find("assert ", 0) == 0)
	{
		...
		return (Print);
	else if (std::regex_match(line, std::regex("^(and)(( )?;.*)?$")))
		return (And);
	else if (std::regex_match(line, std::regex("^(or)(( )?;.*)?$")))
		return (Or);
	else if (std::regex_match(line, std::regex("^(xor)(( )?;.*)?$")))
		return (Xor);
